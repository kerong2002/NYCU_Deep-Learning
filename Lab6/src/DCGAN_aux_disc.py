...
from aux_discriminator import aux_discriminator
...
parser.add_argument('--output_dir', type=str, default='Result_DC_aux/', help='output directory of results')
parser.add_argument('--model_dir', type=str, default='checkpoint_DC_aux/', help='output directory of generator')
parser.add_argument('--test_dir', type=str, default='Test_DC_aux/')
...
class Generator(nn.Module):
    ...
    def forward(self, x, labels):
        x = x.view(-1, 100, 1, 1)
        class_embedding = self.class_embedding(labels).reshape(-1, 100, 1, 1)
...
discriminator = Discriminator(nc, ndf, 24).to(device)
a_discriminator = aux_discriminator()

# if cuda:
generator.to(device)
discriminator.to(device)
a_discriminator.to(device)
...
for epoch in range(opt.n_epochs):
    ...
    gloss = 0
    dloss = 0
    trange = tqdm(enumerate(dataloader), total=len(dataloader), desc="DCGAN_aux")
    acc = 0
    generator.train()
    discriminator.train()
    for i, (imgs, labels) in trange:
        ...
        aux_loss = nn.BCELoss()(a_discriminator(gen_imgs), labels)
        g_loss = adversarial_loss_g(validity) +  aux_loss
